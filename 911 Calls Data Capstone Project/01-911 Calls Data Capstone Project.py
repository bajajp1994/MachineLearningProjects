...
df2=df1.apply(lambda x:x.split(':'))
...
sns.countplot(x='DayOfWeek',data=df,hue='Reasons',palette='viridis')
...
d=df.groupby(by=['DayOfWeek','hour']).count()['Reasons'].unstack().head()
d
...
d=df.groupby(by=['DayOfWeek','month']).count()['e'].unstack()
d
